...
// The pick numbers drawn over chosen faces: each pick re-points that face's
// menu picture at one of the game's own controller-number textures (the
// yellow 1-4 seen over heads in a match), the same trick the game uses for
// the little sidekick head. Undone pick by pick when B rewinds.
static FETextureResource gPickerDigitRes[5];

static const char* const kPickerDigitTexNames[5] = {
    NULL,
    "fe/controller_1_indicator",
    "fe/controller_2_indicator",
    "fe/controller_3_indicator",
    "fe/controller_4_indicator",
};
...
static void PickerNumberFace(IChooseCaptain* p, int side, int pickIdx, bool onCaptainGrid, const char* cellName)
{
    ...
    unsigned long tex = glGetTexture(kPickerDigitTexNames[pickIdx + 1]);
    if (!glTextureLoad(tex))
    {
        return; // not resident; leave the face as it is rather than draw garbage
    }
...
// The Super Strikes option, shown on the Y view and flipped with X. The text
// rides in the screen's own SIDEKICK_NAME line, so it uses the game's font.
static void PickerShowSuperOption(IChooseCaptain* p, int side)
{
    static const unsigned short kAll[] = {
        'S','U','P','E','R',' ','S','T','R','I','K','E','S',':',' ',
        'E','V','E','R','Y','O','N','E',' ','(','X',')',0};
    static const unsigned short kCaptain[] = {
        'S','U','P','E','R',' ','S','T','R','I','K','E','S',':',' ',
        'C','A','P','T','A','I','N','S',' ','O','N','L','Y',' ','(','X',')',0};

    IChooseCaptain::NameComponent* nc = &p->mNameComponents[side];
    TLTextInstance* pText = FEFinder<TLTextInstance, 2>::Find<TLSlide>(
        nc->mComponent->GetActiveSlide(),
        InlineHasher(nlStringLowerHash(nc->mSidekickObjName)));
    if (pText != NULL)
    {
        bool all = GetConfigBool(Config::Global(), "super_all", false);
        pText->SetString(all ? kAll : kCaptain);
    }
}
...
UpdateResult IChooseCaptain::Update(float dt)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            // MOD (mixed teams): X, on the Y view, flips who may Super Strike.
            int xside = GetSide(inputpad);
            if (MixedPickerOn() && xside >= 0
                && mComponentState[xside].mCurrentPhase == PHASE_CHOOSING_SIDEKICK)
            {
                Config& cfg = Config::Global();
                ...
                PickerShowSuperOption(this, xside);
                FEAudio::PlayAnimAudioEvent("sfx_accept_no_screen_change", false);
            }
        }
        else
